...
logic — only add the guarantees below. Keep env values (URLs, EXECUTE_PATH, pool sizes)
...
- The database (owned by Python) is the source of truth for "did this execute?". The
  Java/Hazelcast flag is a projection that can be rebuilt from it.

## 1. Idempotency key
- Use the existing per-execution / per-resolve id (confirmed 1:1 with one intended action).
- It MUST be stable across retries (from the request/record, never a fresh UUID/timestamp per
  call) and unique per execution.
...
## 3. Verify updateAiResolutionFlag is doing the right thing (ALL must hold)
   ...
   5. No clobber: addStepVariables(..., replaceAll=false); only ONE writer per step at a time.
...
## 4. RestClient connection pool (Apache HttpClient 5)
   ...
   - BOOT 4 GOTCHA: Spring Boot 4 removed ClientHttpRequestFactorySettings /
     ClientHttpRequestFactoryBuilder — use HttpComponentsClientHttpRequestFactory as above.
   Reference (verified to build/run on Spring Boot 4 + Java 21+):
     ...
     RestClient restClient = RestClient.builder().requestFactory(factory).baseUrl(pythonBaseUrl).build();

## 5. Hazelcast single-writer (cluster-wide)
   addStepVariables(..., replaceAll=false) is a read-merge-write. Ensure ONE writer per step:
   before executing, do an atomic claim IMap.putIfAbsent(idempotencyKey, "IN_PROGRESS") and
   proceed only if it returns null. Treat this as an optimisation; the DB constraint on the
   Python side is the real guarantee.

## Acceptance criteria (Java side)
   1. Two executes with the same Idempotency-Key -> action runs once; second returns same response.
   2. execute succeeds but response lost (retry) -> one action; flag EXECUTED.
   3. crash between execute and updateAiResolutionFlag, then re-run -> no double action; flag terminal.
   4. two concurrent executes for one key across nodes -> exactly one action (putIfAbsent + DB block dup).
   5. execute fails -> flag EXECUTION_FAILED, never EXECUTED.
...
## TODO (later — separate change, not this one): pickup re-drive
   Ensure the pickup mechanism (reads a stored Python result from Hazelcast and calls
...
below. Keep env values (DB config, ports) configurable.
...
## 3. Database constraint (the real guarantee)
   - Add a UNIQUE constraint on the idempotency id (or use INSERT ... ON CONFLICT (<id>) DO
     NOTHING), so a duplicate row is physically impossible even under a concurrent race that
     slips past the in-memory dedupe check.

## 4. Result must be retrievable / authoritative
   - The stored result (keyed by idempotency id) is the source of truth. It must be persisted so
     it survives a restart and can answer "was this executed, and what was the outcome?" — Java's
     flag is only a projection of this.
...
Both share one contract — the Idempotency-Key header on the same per-execution id — so if each side implements its half, the exactly-once guarantee holds end to end. Want a one-line summary of that shared contract to paste at the top of each ticket?
